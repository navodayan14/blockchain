...
{  
   var success=await instance.transfer(accounts[2],tokens('1000'),{from:accounts[0]});
  ...
  // assert(success==true,"return true");
   ...
   assert.equal(_toaccount.toString(),tokens('1000'),"_to account has credited");
});
...
{
   var reciept=await instance.transfer(accounts[3],tokens('100'),{from:accounts[0]});
...
it("transfer modifier are working",async function(){
    try{
var success= await instance.transfer(accounts[3],tokens('98901'),{from:accounts[0]});
 throw('wrong!!');
}
catch(err)
{
...
var reciept =await instance.approve(accounts[4],tokens('100'),{from:accounts[2]});
...
it('approval modifier are working ',async function(){
    ...
    try{
    var reciept =await instance.approve(accounts[4],tokens('1001'),{from:accounts[2]});
    throw("wrong!!");
    }
    catch(err){
       assert( err.message.includes(" not adequate balance"));
...
let reciept = await instance.tranferFrom(accounts[2],accounts[3],tokens('80'),{from:accounts[4]});
...
it("transferfrom will allow only correct amount",async function()
{
    ...
    try{
        var reciept =await instance.tranferFrom(accounts[2],accounts[3],tokens('21'),{from:accounts[4]});
        throw("wrong!!");
        }
        catch(err){
